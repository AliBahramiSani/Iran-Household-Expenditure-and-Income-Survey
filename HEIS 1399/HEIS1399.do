...
clear
**  99Data  **
odbc load, table("U99Data") lowercase
destring _all, replace force
gen urban=1
save U99Data.dta, replace
clear
odbc load, table("R99Data") lowercase
destring _all, replace force
gen urban=0
save R99Data.dta, replace
append using U99Data
save 99Data.dta, replace
clear

** 99P1 **
odbc load, table("U99P1") lowercase
...
save U99P1.dta, replace
clear

odbc load, table("R99P1") lowercase
destring _all, force replace
gen long ostan = floor( address /100000000)
...
label define urban 0 "Rural" 1 "Urban"
label values urban urban
save R99P1.dta, replace
append using U99P1
save 99P1.dta, replace

** 99P1 and 99Data -> 99Demographics **
merge m:m address using 99Data, nogen
...
drop WA
save 99Demographics.dta, replace
...
use 99Demographics
forvalues i = 1/9{
	merge m:m address using 99P3S0`i', nogen
}
forvalues i = 11/14{
	merge m:m address using 99P3S`i', nogen
}
forvalues i = 1/9{
...
save IndividualExpenditure99.dta, replace
/*
   Household Level
*/
clear
...
clear
** 99P2 **
odbc load,  table("U99P2") lowercase
...
save U99P2.dta, replace

clear

odbc load, table("R99P2") lowercase 
ren dycol01 HomeOwnership
ren dycol03 NoRooms
ren dycol04 HouseArea
ren dycol05 HouseStructure
...
ren dycol43 CookingFuel
ren dycol44 HeatingFuel
save R99P2.dta, replace
append using U99P2 
destring _all ,replace force
foreach var of varlist HomeOwnership-HeatingFuel{
	replace `var' = 0 if `var'==.
}
save 99P2.dta, replace

merge m:m  address using 99P1, nogen
...
ren county countyname
merge 1:1 countyname using CountyMDPI99, nogen
save CountyMDPI99, replace

**** Deciling
* Building Headcount
use 99Demographics
gen Headcount = 1 if Rel2Head ==1
replace Headcount=0.5 if Age <=18
replace Headcount=0.7 if Headcount==.
collapse (sum) Headcount, by( address)
merge 1:1 address using HouseholdExpenditure, nogen
save HouseholdExpenditure.dta, replace
* Expenditure Deciles of Household
use HouseholdExpenditure
gen NonDurableExpenditures = (ServicesOtherExpenses + HotelingRestExpenses + RecreationExpenses + CommunicationExpenses + TransportationExpenses + HomeApplianceExpenses + HousingExpenses + ClothesExpenses + FoodExpenses)/Headcount
xtile ExDecile = NonDurableExpenditures [pweight = weight], nq(10)